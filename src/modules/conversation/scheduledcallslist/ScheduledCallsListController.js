...
var ScheduledCallsListController = {
	viewController: null,
	chatRef: null,
	nodes: [],
	updateState() {
	...
	getData() {
		...
		this.chatRef.orderByChild("from_type").equalTo("call").on('value', (snapshot) => {
			if(!!snapshot.val()) {
		  		ScheduledCallsListController.nodes = _.values(snapshot.val());
		  		this.updateState();
		  	}
		  	this.hideModal();
	    });
...
module.exports = ScheduledCallsListController;